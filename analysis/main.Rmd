...
PeopleReactionGrouped <- function(data, question_id, grouping_var, gndr_filter = FALSE) {
  share_calculation <- data %>% 
    filter(Question == question_id & 
             !is.na(Answer) & 
  ...
  share_calculation %>% 
    ...
    # spread for datawrapper
    spread(key = year, value = share) %>% 
    write_csv(path = glue::glue("output/approvement_{question_id}_{grouping_var}.csv"))
  ...
  addendum <- ifelse(gndr_filter == "Mann", glue::glue("{addendum} Männern)"), addendum)
  addendum <- ifelse(gndr_filter == "Frau", glue::glue("{addendum} Frauen)"), addendum)
  ...
  share_calculation %>% 
    ...
    theme_srf() +
    labs(title = translate_question_ids_short(question_id),
         subtitle = ifelse(grouping_var == "icsbfm", glue::glue("Frage jeweils betreffend Frauen und Männern separat gestellt {addendum}"), ""),
        x = "", y = "")
}
